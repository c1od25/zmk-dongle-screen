...
static void rain_status_refresh(void) { rain_rebuild_lut(); }

ZMK_DISPLAY_WIDGET_LISTENER(widget_rain_status, struct rain_status_state, rain_status_update_cb,
                            get_state)
...
int zmk_widget_rain_status_init(struct zmk_widget_rain_status *widget, lv_obj_t *parent) {
    ...
    if (widget->timer == NULL || widget->gate_timer == NULL) {
        ...
        LOG_ERR("rain widget: timer allocation failed (frame=%p gate=%p)", (void *)widget->timer,
                (void *)widget->gate_timer);
        if (widget->timer != NULL) {
...
lv_obj_t *zmk_widget_rain_status_obj(struct zmk_widget_rain_status *widget) { return widget->obj; }
