...
static void set_mod_key_active(lv_obj_t *key, lv_obj_t *icon, bool active) {
    if (active) {
        lv_obj_set_style_bg_color(key, lv_color_darken(theme_accent_color(), 190), LV_PART_MAIN);
...
int zmk_widget_mod_status_init(struct zmk_widget_mod_status *widget, lv_obj_t *parent) {
    ...
    widget->shift =
        mod_key_create(widget->obj, &mod_key_pos[0], MOD_ICON_SHIFT, &widget->shift_icon);
