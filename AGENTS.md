...
Zephyr module + ZMK shield for a dongle display (ST7789V 240x280 + LVGL) on split-BLE keyboards. Fork of janpfischer/zmk-dongle-screen, deeply customized for ErgoAstra: 9 LVGL widgets, sleep-state accent fade, battery filtering, custom ST7789V driver.
...
- **`ZMK_DISPLAY_WIDGET_LISTENER` + `ZMK_SUBSCRIPTION`** is the standard widget pattern (6 of 9 widgets). Exceptions: `mod_status` (100ms lv_timer polling HID report), `scanner_status` (60ms animation timer), `gif_status` (lv_gif internal timer, no events).
...
- **`usb_present` in battery_status struct is dead** — written but never rendered.
...
- **Font glyphs are minimal sets** — new icon requires regenerating the font C file with the new PUA codepoint in `--symbols`; codepoints above BMP need `\U0000XXXX` (8-digit) escape.
- **scanner trail colors assume background 0x0a0a0d** — if panel bg changes, update `SCAN_BG_R/G/B` in `scanner_status.c`.
...
- **Two kinds of "sleep"**: screen sleep (backlight off, in `brightness.c`) vs keyboard sleep (both halves' battery < 1, in `sleep_status.c` + `theme.c`). They are independent.
- **Battery widget is the only "connection sensor"** — wake-on-reconnect, sleep detection, L/R tags all derive from battery level events, not BLE connection events.
